...
def get_fertilizer_recommendation(nitrogen, phosphorous, potassium, cropname):
    # Determine the advice for each nutrient
    n_advice = 'NHigh' if nitrogen >= 50 else 'Nlow'
    p_advice = 'PHigh' if phosphorous >= 50 else 'Plow'
    k_advice = 'KHigh' if potassium >= 50 else 'Klow'

    # Get the specific advice for each nutrient level
    n_recommendation = fertilizer_dic[n_advice]
    p_recommendation = fertilizer_dic[p_advice]
    k_recommendation = fertilizer_dic[k_advice]

    # Combine the recommendations into a single advice string
    recommendation = f"Nitrogen: {n_recommendation}<br><br>Phosphorous: {p_recommendation}<br><br>Potassium: {k_recommendation}"
